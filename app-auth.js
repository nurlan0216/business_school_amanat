...
function triggerInstantBlock(reason) {
  ...
  setTimeout(() => {
    ...
    const OVERLAY_DATA = {
      ...
      noaccess: {
        icon:  '⏸️',
        title: { ru: 'ДОСТУП ПРИОСТАНОВЛЕН',        kz: 'ҚОЛЖЕТІМДІЛІК ТОҚТАТЫЛДЫ' },
        text:  { ru: 'Ваш доступ к образовательной платформе приостановлен. Возможно, истёк срок оплаты. Обратитесь в техподдержку.',
                 kz: 'Білім беру платформасына қолжетімділігіңіз тоқтатылды. Төлем мерзімі өткен болуы мүмкін. Техқолдауға хабарласыңыз.' },
        waMsg: { ru: 'Здравствуйте, мой доступ к платформе приостановлен. Прошу восстановить.',
                 kz: 'Сәлеметсіз бе, платформаға қолжетімділігім тоқтатылды. Қалпына келтіруді сұраймын.' }
      },
      notfound: {
        icon:  '🔴',
    ...
    const d    = OVERLAY_DATA[reason] || OVERLAY_DATA.noaccess;
    const l    = isKz ? 'kz' : 'ru';
